...
class POLLUXCMFGENProcessGrid(BaseProcessGrid):
    """
    ...
    def __init__(self, index, input_wavelength, meta, wavelength_start=0*u.angstrom, wavelength_stop=np.inf*u.angstrom,
        ...
        R_sampling : integer
        """
        super(POLLUXCMFGENProcessGrid, self).__init__(index, input_wavelength, meta, wavelength_start=wavelength_start,
                                                 wavelength_stop=wavelength_stop, R=R, R_sampling=R_sampling)




    def interp_wavelength(self, flux, debug=False):
        ...
        if debug:
            print("cut_wavelength shape:", self.cut_wavelength.shape)
            print("processed_flux shape:", processed_flux.shape)
